...
namespace OWSData.Repositories.Implementations.Postgres
{
    ...
    public class UsersRepository : IUsersRepository
    {
        ...
        public async Task<SuccessAndErrorMessage> RegisterUser(Guid customerGUID, string email, string password)
        {
            SuccessAndErrorMessage outputObject = new SuccessAndErrorMessage();
            ...
            try
            {
                using (Connection)
                {
                    ...
                    await Connection.ExecuteAsync("select * from AddUser(@CustomerGUID, @Email, @Password, @Role)",
                        p,
                        commandType: CommandType.Text);
                }
